...
void NdpPullPacer::sendPacket(Packet* ack, NdpPacket::seq_t rcvd_pacer_no, NdpSink* receiver) {
    ...
    */
	...
	// turn leaving NACK packets to PULL packets (NACK packets are now generated by switches and they are not implicit pulls)
	if (ack->type() == NDPNACK) {
		NdpPull *pull_pkt = NdpPull::newpkt((NdpNack*)ack);

		if (_pull_queue.empty()) {
			simtime_picosec delta = eventlist().now() - _last_pull;
			if (delta >= drain_time) {
				set_pacerno(pull_pkt, _pacer_no++);
				pull_pkt->flow().logTraffic(*pull_pkt, *this, TrafficLogger::PKT_SEND);
				pull_pkt->sendOn();
